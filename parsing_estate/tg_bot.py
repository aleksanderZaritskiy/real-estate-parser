...
from telegram.ext import MessageHandler, Filters, Updater, CommandHandler, CallbackContext, CallbackQueryHandler, ConversationHandler
from telegram.error import BadRequest

from .constants import TEGELGRAM_TOKEN, INPUT_LOCATION, INPUT_ALLREADY_EXISTS, INPUT_MAIL, CONFIG, PATTERN_VALID_MAIL
from .utils import transform_input_answer, place, converter_time
from .exceptions import PunctuationCharError, LatinLettersLocationError, NumberInLocationError, UncorrectTimeCreateAd, ImportantConfigsDoNotExists
...
def button(update: Type[Update], context: Type[CallbackContext]) -> None:
    ...
    logging.info(f'Пользователь указывает настройки к: {cur_option} выбор: {transform_input_answer(cur_option, [query.data])}')
    ...
    for index in range(len(keyboard)):
        ...
        if ans in current_param_data['input_answer'] and text == 'Готово':
            ...
            if cur_option == 'send_parsing_file' and '0' in current_param_data['input_answer']:
                query.edit_message_text(f'Введите почту на которую отправлять данные. Что бы завершить настройки почты введите "/ok"')
            else:
    ...
    try:
        query.edit_message_text(text=current_param_data['text_message'], reply_markup=reply_markup)
    except BadRequest:
        query.message.reply_text(current_param_data['text_message'], reply_markup=reply_markup)


def area_command(update: Type[Update], context: Type[CallbackContext]) -> NoReturn:
    keyboard = [
        [InlineKeyboardButton("avito.ru", callback_data='0'),
         InlineKeyboardButton("cian", callback_data='1')],
        [InlineKeyboardButton("Готово", callback_data='2')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text('Выберите площадки для поиска, затем нажмите "Готово":', reply_markup=reply_markup)


    context.user_data['params'] = 'area'
    context.user_data['area'] = {
        'keyboard': [[InlineKeyboardButton(f"{place}", callback_data=str(i))] for i, place in enumerate(['avito.ru', 'циан', 'Готово'])],
...
def rooms_count_command(update: Type[Update], context: Type[CallbackContext]) -> NoReturn:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text('Выберите количество комнат, затем нажмите "Готово":', reply_markup=reply_markup)


    context.user_data['params'] = 'rooms_count'
    context.user_data['rooms_count'] = {
        'keyboard': [[InlineKeyboardButton(f"{place}", callback_data=str(i))] for i, place in enumerate(['апартаменты', '1 комната', '2 комнаты', '3 комнаты', '4 комнаты', '5 комнат и более', 'свободная планировка', 'Готово'])],
        'text_message': "Выберите количество комнат, затем нажмите 'Готово':",
        'input_answer': [],
    }


def send_parsing_file_command(update: Type[Update], context: Type[CallbackContext]) -> int:
    keyboard = [
        [InlineKeyboardButton("почта", callback_data='0'),
         InlineKeyboardButton("чат с ботом", callback_data='1')],
        [InlineKeyboardButton("Готово", callback_data='2')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text('Выберите место для выгрузки файла с собранными данными, затем нажмите "Готово":', reply_markup=reply_markup)


    context.user_data['params'] = 'send_parsing_file'
    context.user_data['send_parsing_file'] = {
        'keyboard': [[InlineKeyboardButton(f"{place}", callback_data=str(i))] for i, place in enumerate(['почта', 'чат с ботом', 'Готово'])],
        'text_message': "Выберите площадки для поиска, затем нажмите 'Готово':",
        'input_answer': [],
    }   
    return INPUT_MAIL
...
def input_mail(update: Type[Update], context: Type[CallbackContext]) -> NoReturn:
    ...
    if match:
        ...
        update.message.reply_text(f'Вы указали почту {text}. Корректно? Если да - введите "/ok", если нет - Введите почту ещё раз')
    else:
...
def end_input_mail(update: Type[Update], context: Type[CallbackContext]) -> int:
    if not context.user_data['send_parsing_file'].get('accepted_mail'):
        ...
        update.message.reply_text(f'Настройки почты не приняты. Если хотите выбрать способ отправки файлов - "почта", введите команду "/send_parsing_file -> "почта" и укажите валидную почту"')
    else:
...
def input_location(update: Type[Update], context: Type[CallbackContext]) -> NoReturn:
    ...
    try:
    ...
    except (PunctuationCharError, LatinLettersLocationError, NumberInLocationError) as error:
        ...
        update.message.reply_text(f'Указанный город {text} принят. Если хотите изменить, введите город повторно, для завершения - команда "/ok"')


def end_input_location(update: Type[Update], context: Type[CallbackContext]) -> int:
    if context.user_data['location']['input_answer'] == 'all/':
        update.message.reply_text('Название населенного пункт не принятно. Локация остаётся "по всей стране". Для повторного набора вызовите команду "/location"')
    return ConversationHandler.END


def already_exists_command(update: Type[Update], context: Type[CallbackContext]) -> int:
    update.message.reply_text('Укажите временную дельту отбора объявлений.\nНапример "30 минут", "1 час", "2 недели".\nБот будет добавлять объявления, которые размещены до указанной дельты(включительно).\nТакже вы можете указать дату и время.\nНапример "15 января 18:35". Принцин добавления объявлений будет тот же.\nЧтобы завершить /ok')
    return INPUT_ALLREADY_EXISTS


def input_already_exists(update: Type[Update], context: Type[CallbackContext]) -> NoReturn:
    text = update.message.text
    context.user_data['already_exists'] = {'input_answer': datetime.datetime(1999, 1, 1, 12, 0, 0)}
    try:
        ...
        logging.error(f'{error.__class__.__name__} : {error.args}')
        update.message.reply_text(f'{text} Некорректный формат дельты. Ознакомьтесь с инструкцией ввода')
    else:
        context.user_data['already_exists'] = {'input_answer': current_time}
        update.message.reply_text(f'Временная дельта {text} принят. Если хотите изменить, введите значение повторно, для завершения - команда "/ok"')


def end_input_already_exists(update: Type[Update], context: Type[CallbackContext]) -> int:
    if not context.user_data.get('already_exists') or datetime.datetime(1999, 1, 1, 12, 0, 0) == context.user_data['already_exists']['input_answer']:
        update.message.reply_text('Временная дельта не принята. Поиск остаётся по объявлениям "за весь период". Для повторного набора вызовите команду "/already_exists"')
    return ConversationHandler.END
...
def start_command(update: Type[Update], context: Type[CallbackContext]) -> None:
    ...
    if chat_obj:
        ...
        params = [('area', ''), ('rooms_count', ''), ('location', ''), ('already_exists', ''), ('send_parsing_file', 'accepted_mail')]    
        for param, specific in params:
            if param in context.user_data:
                if param == 'send_parsing_file' and context.user_data[param].get(specific):
        ...
        update.message.reply_text(f'Сбор данных займет время. Результат будет выслан в соответствии с указанными настройками. Ожидайте')
        return    
    try:
        area = context.user_data['area']['input_answer']
        rooms_count = context.user_data['rooms_count']['input_answer']
        if not area or not rooms_count:
            raise ImportantConfigsDoNotExists('Не указаны настройки для /area или для /rooms_count')
    except KeyError as error:
        logging.info(f'{error.args}')
        update.message.reply_text(f'Не указана конфигурация для: {error.args}')
        return
    data = {'accepted_mail': 'не указано'}
    params = [('location', ''), ('already_exists', ''), ('send_parsing_file', 'accepted_mail')]
    for param, specific in params:
        if param in context.user_data:
            if param == 'send_parsing_file' and context.user_data[param].get(specific):
                data[param] = context.user_data[param].get('input_answer')
                data[specific] = context.user_data[param].get(specific)
            else:
                data[param] = context.user_data[param].get('input_answer')
        else:
...
def tg_bot(queue: Type[Queue]) -> NoReturn:
    ...
    conv_handler_location = ConversationHandler(
        ...
        states={
            INPUT_LOCATION: [MessageHandler(Filters.text & ~Filters.command, input_location)]
        },
    ...
    conv_handler_already_exists = ConversationHandler(
        ...
        states={
            INPUT_ALLREADY_EXISTS: [MessageHandler(Filters.text & ~Filters.command, input_already_exists)]
        },
